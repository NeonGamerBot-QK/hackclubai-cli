...
if (!cmd) {
  console.error(
    "No command provided, possible commands are: help, hey, stats, chat",
  );
  Deno.exit(1);
}
...
if (!validCmds.includes(cmd.toLowerCase())) {
  console.error(
    "Invalid command, possible commands are: help, hey, stats, chat",
  );
  Deno.exit(1);
}
...
if (cmd == "stats") {
  fetch("https://ai.hackclub.com/", {
    ...
    .then((text) => {
      ...
      console.log(
        `The model has been used ${bold(usageCount)} times and is ${bold(model)}`,
      );
    });
}
if (cmd == "chat") {
    ...
    fetch("https://ai.hackclub.com/chat/completions", {
      ...
      body: JSON.stringify({
        messages: [
          {
            role: "user",
            content: `Previous messages: ${chatLogs.map((e) => `${e.content} said by ${e.role}`).join("\n")}\ncurrent message: ${line.trim()}`,
          },
        ],
      }),
    })
